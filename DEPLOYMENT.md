...
5. **Verify before moving on:**
    ...
    ```
    A Job that keeps retrying with a connection error means `externalDatabase.host` doesn't match the Service name in `local-test/postgres.yaml`.
