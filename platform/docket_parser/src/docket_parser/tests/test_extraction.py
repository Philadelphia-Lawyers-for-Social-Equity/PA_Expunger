...
class TestDocketExtraction:
    ...
    test_file_paths = test_data_path.glob("*.pdf")
    ...
    def test_warn_not_crystal_report(self, caplog):
        ...
        file_path = self.test_data_path.joinpath("not_crystal_reports.pdf")
        with caplog.at_level("WARNING", logger_name):
            extraction.DocketReader(file_path)
        assert any("Crystal Reports" in msg for msg in caplog.messages), \
            "DocketReader warns when metadata does not indicate the file is generated by Crystal Reports"
